...
def loadData(fileName):
    df = pd.read_csv(fileName).drop(columns=['Date', 'Time'],axis = 1)
    for i in range(df.shape[0]):
        if df['Item'][i]=='NONE':
            df = df.drop(index=i, axis=1)
    sector = df.groupby('Transaction')
    df2 = pd.DataFrame(columns=['Transaction','ItemList'])
    indexs = list(df['Transaction'].unique())
    for i in indexs:
        df2.loc[i] = [i, list(sector.get_group(i)['Item'].unique())]
        print(df2.loc[i])
    return df2
...
df = loadData('BreadBasket_DMS.csv')
...
with open('patern_BreadBasket.txt', 'w') as f:
    for p in total_all: 
        out_str = '('
        for item in p[:-2]:
